...
// ── STOCKFISH WORKER ──────────────────────────────────────────
//  stockfish.js reponun kök dizininde olmalı (nmrugg pure-JS versiyonu).
//  https://raw.githubusercontent.com/nmrugg/stockfish.js/master/stockfish.js

function initStockfish() {
  setStatus('loading');
  document.getElementById('status-text').textContent = 'Yükleniyor…';

  let worker;
  try {
    worker = new Worker('./stockfish.js');
  } catch (e) {
    engineFailed('Worker oluşturulamadı: ' + e.message);
    return;
  }
  ...
  const readyTimer = setTimeout(() => {
    if (!sfReady) {
      worker.terminate();
      engineFailed('30sn içinde yanıt gelmedi. stockfish.js dosyasını nmrugg versiyonu ile değiştir.');
    }
  }, 30000);

  worker.onmessage = (e) => {
    // lichess/nmrugg versiyonları bazen event.data, bazen düz string döner
...
function engineFailed(reason) {
  ...
  showToast('Motor başlatılamadı — konsolu kontrol et', 'error');
}

...
// ── STOCKFISH WORKER ──────────────────────────────────────────
//  Stockfish11 JS kodunu fetch ediyoruz.
//  İçindeki relative 'stockfish.wasm' referansını tam CDN URL'siyle patch'liyoruz.
//  Böylece blob worker da WASM dosyasını bulabiliyor.

const SF_BASE = 'https://cdn.jsdelivr.net/gh/nmrugg/stockfish.js@Stockfish11/src/';
const SF_JS   = SF_BASE + 'stockfish.js';
const SF_WASM = SF_BASE + 'stockfish.wasm';

function initStockfish() {
  setStatus('loading');
  document.getElementById('status-text').textContent = 'İndiriliyor…';
  ...
  fetch(SF_JS)
    ...
    .then(code => {
      ...
      // Relative WASM path → absolute CDN URL
      const patched = code
        .replace(/'stockfish\.wasm'/g,  `'${SF_WASM}'`)
        .replace(/"stockfish\.wasm"/g,  `"${SF_WASM}"`);
      ...
      const readyTimer = setTimeout(() => {
        if (!sfReady) {
          worker.terminate();
          engineFailed('30sn içinde yanıt gelmedi');
        }
      }, 30000);

      worker.onmessage = (e) => {
        const line = typeof e === 'string' ? e : (e.data || '');
        if (line === 'readyok') clearTimeout(readyTimer);
        handleSFMessage(line);
      };

      worker.onerror = (err) => {
        clearTimeout(readyTimer);
        worker.terminate();
        engineFailed('Worker hatası: ' + (err.message || 'bilinmiyor'));
      };
    ...
    .catch(err => {
      engineFailed('Fetch hatası: ' + err.message);
    });
}

function engineFailed(reason) {
  console.error('[ChessLens] Motor hatası:', reason);
  setStatus('error');
  document.getElementById('status-text').textContent = 'Hata';
  showToast('Motor başlatılamadı: ' + reason, 'error');
}
...
function handleSFMessage(raw) {
  const line = typeof raw === 'string' ? raw : (raw?.data || '');
  // ── Engine ready ──
  if (line === 'readyok') {
    sfReady = true;
    setStatus('ready');

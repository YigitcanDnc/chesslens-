...
// ── STOCKFISH WORKER ──────────────────────────────────────────
//  Blob trick: creates a same-origin worker that importScripts
//  from the CDN — bypasses the cross-origin worker restriction.
function initStockfish() {
  ...
  try {
    ...
    setStatus('error');
    showToast('Worker başlatılamadı', 'error');
  }
}

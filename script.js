...
// ── STOCKFISH WORKER ──────────────────────────────────────────
//  Stockfish.js kaynak kodunu fetch ile indirip Blob URL'den
//  worker oluşturuyoruz. importScripts yerine bu yöntem daha güvenilir.
function initStockfish() {
  ...
  fetch(SF_URL)
    ...
    .then(code => {
      ...
      stockfish.onmessage = (e) => handleSFMessage(e.data);
      stockfish.onerror   = (e) => {
        console.error('Stockfish worker error:', e);
        setStatus('error');
        showToast('Motor çalışırken hata oluştu', 'error');
      };
    ...
    .catch(err => {
      ...
      setStatus('error');
      showToast('Stockfish indirilemedi. İnternet bağlantını kontrol et.', 'error');
    });
}
